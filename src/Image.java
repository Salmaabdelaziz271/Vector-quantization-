    ...
    public void newImage(List<Block> replacedBlocks, String outputPath) {
        try {
            ...
            for (Block block : replacedBlocks) {
                ...
                for (int y = 0; y < block.getHeight() && currentX + block.getWidth() < newWidth; y++) {
                    for (int x = 0; x < block.getWidth() && currentX + x < newWidth; x++) {
                        // Convert Double to int before using the left shift operator
                        int pixelValue = blockPixels.get(y).get(x).intValue();
                        resultImage.setRGB(currentX + x, y, pixelValue << 16 | pixelValue << 8 | pixelValue);
                    }
                }
